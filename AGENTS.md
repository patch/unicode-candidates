...
| Proposal requirements, readiness, or research that may change an existing candidate ledger | [Proposal requirements ledger](<Methods/Proposal requirements ledger.md>) |
...
- When both routes are material, keep Character and Emoji research active in parallel even if one submission route is temporarily ineligible. Current Unicode guidance does not accept proposals to emojify existing characters, so resolve the intended submission route before a non-emoji encoding could foreclose later emoji treatment; treat eligibility as a gate on submission, not research priority.
...
- Do not use bare ‘ISO’ as presumed shorthand: name the specific standard or state the concrete syntax, such as `YYYY-MM-DD`.
...
- For an unencoded candidate, do not treat the absence of direct character interchange as evidence against encoding: users cannot interchange a standardized character that does not yet exist. Separate observed current representations or workarounds, demonstrated present need or loss, and prospective adoption or benefit after encoding. Words, images, metadata and higher-level systems establish available alternatives; treat them as adverse only when relevant user or system evidence shows that they satisfy the same prospective operations or that encoding would add no material function.
...
- Balance content authenticity and provenance; open data, research, and standards; freedom of information and common human knowledge; creator rights, copyright, and licensing compliance; safety and harm reduction; and non-pedantic simplicity. Do not turn one value into a blanket exclusion that defeats the others.
...
- Apply the proportionate, source-sensitive managed-risk standard in [Evidence storage and publication](<Project/Evidence storage and publication.md>). Governing licences and agreements, accurate rights notices, and lawful use are mandatory; zero residual risk is not the objective. Do not treat non-free status, an absent release, or depicted people, signs, logos, artwork, packaging, or property as an automatic bar to a truthful descriptive use.
- Prefer open material when it is genuinely equivalent, but preserve the actual artefact when its appearance, occurrence, provenance, or context is itself evidence. Use fit-for-purpose quality and context.
- Keep review simple by default. Record source, creator, rights basis, attribution, use context, and concrete limits; add a fuller rationale only for materially ambiguous or consequential cases. A scoped decision may cover similar assets, derivatives, and in-context distribution channels; reassess material changes, and respond to credible concerns through review, correction, replacement, or withdrawal.
...
- Treat a candidate proposal requirements ledger as a maintained projection of integrated research, not an independent source. Whenever candidate-specific research is integrated and that candidate has a ledger, reconcile every materially affected field in the same task, update its cut-offs and readiness summary where applicable, and identify the changed field IDs in the final handoff; if nothing changes, state `Ledger impact: none`.
- Apply explicit sourced facts, dates, counts, links, technical values and completed artefacts to the ledger when their support is unambiguous. Qualify conflicting or replacement evidence rather than using last-write-wins. Obtain project-owner approval before finalizing changes to character identity, scope, route strategy, readiness, lifecycle phase, consequential rights or privacy judgements, or a decision to submit.
- Run `python3 scripts/validate_proposal_ledgers.py` after ledger changes. Structural validation does not replace source review or substantive judgement.
