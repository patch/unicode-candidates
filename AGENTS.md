...
- Use relative Markdown links rather than Obsidian wiki links in public project notes so navigation works in both Obsidian and GitHub. When a local destination contains spaces, enclose it in angle brackets and retain literal spaces rather than percent-encoding them.
...
- For a public directory linked from repository navigation, use a small `README.md` index and link to that file rather than to the directory itself. This gives GitHub a directory landing page while remaining navigable as an ordinary note in Obsidian.
...
- Write current research and project assets for human collaborators as well as agents. Present the present understanding, material uncertainties, and decisions that affect the work; leave superseded wording and routine changes of direction to Git history or a changelog unless that history is itself evidentially or procedurally relevant.
...
- Use title case for working candidate and project names in research prose and inventories. Reserve all-uppercase character names for formally assigned Unicode names and for explicit proposal-style repertoire or property data. A working name remains provisional regardless of typography.
...
- Capture reusable research methods and cross-project lessons separately from candidate-specific findings so future character or emoji investigations can repeat and improve the process. Research must be capable of supporting, opposing, or leaving unresolved a case for encoding; publish useful findings even when the conclusion is not to propose a character. Maintain a brief refinement log when the method changes.
...
- Give candidates with a weak or negative preliminary case the lowest research priority while retaining them for later evaluation when useful. Do not let low priority predetermine the final conclusion.
...
- The project owner intends to license original project text and assets under CC BY-SA 4.0, while excluding third-party reference material governed by other terms. Use `LICENCE.md` for the repository-level notice. Record exact Creative Commons licences, attribution and share-alike duties, and compatibility before reuse. The project owner can license Shutterstock assets; record the asset ID and applicable licence and do not assume the subscription permits repository redistribution.
- In public project materials, use the registered-mark symbol only in the trademark notice and do not mention the Unicode Logo unless the project actually displays it.
...
- Distinguish the character-proposal and emoji-proposal routes without contrasting emoji with plain text: emoji characters and emoji sequences are plain text. Some candidates may be viable through the character-proposal route, some only through the emoji-proposal route, and some through both. Link shared evidence rather than duplicating it, and do not imply that every candidate needs or qualifies for both routes.
