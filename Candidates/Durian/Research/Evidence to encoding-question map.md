...
| Pending private photo-corpus audit | Contextual only until audited | Contextual to form and Distinctiveness; not Frequency evidence | No finding may be inferred from unaudited material | Catalogue, provenance, form coding and rights review |
| Rights-cleared versus reference-only evidence | Contextual only | Material to image-evidence and proposal requirements, not selection merit by itself | Strong for current publication-readiness distinctions | Item-level clearance and route-specific publication selection after the audit |
...
- **Likely proposal use:** **Character:** core Usage evidence if the independence and identity tests hold; reproduce only a small rights-cleared set and present the prohibition overlay as composition. **Emoji:** distinguish the generic fruit from a signage identity; do not count sign occurrences as Frequency evidence.
...
- **Likely proposal use:** **Character:** only audited, independently probative and appropriately cleared items should enter the usage evidence set. **Emoji:** audited forms may inform Distinctiveness and example-image design, but the corpus does not replace Frequency evidence.
...
- **Likely proposal use:** **Character:** reproduce a small, representative, independently probative and rights-cleared set; cite other sources without reproducing their images where appropriate. **Emoji:** ensure that proposed example images meet the current Image Rights & Licenses requirements and separately clear any reproduced supporting evidence.
...
6. **Collect only decision-bearing gaps.** Commission or seek rights clearance for sign evidence that fills a missing venue, territory, form, independence or workflow cell. Do not accumulate images or contextual statistics as an implicit vote.
...
- **Most consequential unresolved dependency:** For the primary Character strategy, the private photo-corpus audit. It can materially change the assessments of independence, copying, form convergence, semantic boundary, recognizability and publication readiness, and it is the necessary evidence base for a proportionate interchange and counterargument audit. The Emoji route has a separate procedural dependency on official confirmation of re-review eligibility.
