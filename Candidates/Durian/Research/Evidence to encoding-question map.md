...
| Repeated functional prohibition signage | Supports encoding | Contextual and boundary-sensitive: the evidence venue is not necessarily the proposed identity, but a signage identity would be automatically declined | Strong for verified functional occurrence; moderate for a reusable abstract-unit inference | No demonstrated plain-text operation or Emoji usage-level evidence |
| Geographic breadth of observed symbol use | Supports encoding | Indirect research lead for language and geographic frequency analysis | Moderate for regional distribution; limited for prevalence or independent textual use | Sparse occurrences, venue imbalance and no prescribed Emoji frequency data |
...
| Plain-text public-interchange need | Supports non-encoding provisionally | No corresponding Emoji criterion; do not transfer the gap | Strong as a documented Character-route gap, not proof that no need exists | Named users, systems, operations and losses |
...
- **Direction:** **Character:** supports encoding. **Emoji:** contextual and boundary-sensitive; the occurrences do not supply the prescribed usage-level evidence, and an identity defined as signage would be adverse.
...
- **Direction:** **Character:** supports non-encoding provisionally. **Emoji:** not applicable as a selection test.
- **Current strength:** Strong as a gap in the present Character case and therefore as a reason not to draft or submit that proposal; it is not evidence that such workflows do not exist and does not decide Emoji selection.
...
- **Likely proposal use:** **Character:** none until affirmative workflow evidence exists; if a bounded audit still finds no loss, this becomes a central reason not to submit. **Emoji:** no direct use as a selection factor.
...
- **Canonical source:** [Durian signage](<Durian signage.md>), [Durian visual use](<Durian visual use.md>), [Contributor rights](<Contributor rights.md>), [Evidence storage and publication](<../../../Project/Evidence storage and publication.md>), and the current official route guidance [U06](Sources.md#u06) and [U07](Sources.md#u07).
...
- **Strongest current support:** For the Character route, repeated functional durian-prohibition signage with the same regulatory meaning is traceably documented across six Southeast Asian territories and several venue types. This is the best evidence that the referent participates in a stable visual convention rather than appearing only as decoration. For the Emoji route, the absence of an existing Durian representation and the referent’s durable cultural footprint are potentially favourable, but neither yet demonstrates Usage level, Frequency or Distinctiveness.
- **Strongest current weakness:** For the Character route, the project has not demonstrated a public need to preserve and interchange a generic durian unit in plain text. Every reviewed occurrence can still be explained as a physical sign, image, logo, existing letter or punctuation mark, ordinary wording or higher-level composition. For the Emoji route, current prescribed Frequency evidence and a tested Distinctiveness case are absent, and re-review eligibility remains unresolved.
