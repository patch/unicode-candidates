...
This note is a compact register and decision aid for third-party content sources that are realistically available to the project. It keeps source availability and provider-specific acquisition questions out of the general [evidence-storage policy](<Evidence storage and publication.md>). It is not an exhaustive digest of any provider’s terms, a substitute for the governing agreement, or legal advice.
...
Record a provider, collection, contributor route, or other source only when it is realistically available or repeatedly relevant. State the available content type, access context at a safe level, likely project use, and date checked. Availability, access, or a subscription does not mean that the project has acquired or used an asset.

When availability depends on private circumstances, record only the operational consequence needed by collaborators: what content is available, whether the opportunity to make new acquisitions may end, and when that status was checked. Keep the reason, account relationship, entitlement evidence, and other sensitive context private.

### Provider-specific licensing issues

Record only the small number of provider or product questions that could immediately change an acquisition, integration, or publication decision. Link to the governing terms and state when they were checked; do not reproduce a licence agreement or attempt to anticipate every product and use.
...
When a strong evidence source may cease to be available for new licensing:

- prioritize searching it while access remains available, especially for evidence that could materially affect a candidate decision or later proposal;
...
No Shutterstock asset is currently recorded as acquired, incorporated, or published by this project. Add links here only after item-level acquisition and use records have been created with the relevant candidate or evidence item.

## Adding another source

Add a concise provider or source section only when there is a real access route, recurring research value, or an actual acquisition question. Record:

- source and available content types;
- access or acquisition context at a safe level;
- the few provider-specific checks material to this project;
- the governing-terms link and last-check date; and
- links to item-level records for any assets actually acquired or used.

Create a separate provider note only when its project-relevant controls become too substantial for a concise section here.
